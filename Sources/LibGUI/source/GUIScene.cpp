...
{
    MouCa::assertion(std::find(_pages.cbegin(), _pages.cend(), newPage) == _pages.cend());  // DEV Issue: already in scene ?
    _pages.push_back(newPage);
}

void Scene::removePage(PageSPtr& page)
{
    MouCa::assertion(std::find(_pages.cbegin(), _pages.cend(), page) != _pages.cend());  // DEV Issue: not in scene
    ...
    if (!_activePage.expired())
    {
        ...
        for(const auto& widget : page->getWidgets())
        {
            ...
            if (widget2D != nullptr)
            {
                ...
                MouCa::assertion(0 <= dirH && dirH < directions.size()
