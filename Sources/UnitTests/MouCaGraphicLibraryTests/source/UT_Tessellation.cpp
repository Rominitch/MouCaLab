...
class TessellationTest : public MouCaGameEngine3DTest
{
public:
    ...
    static const BT::CPUMemory _nbFrustrumPlanes = 6;
    struct UBOCamera
    {
        glm::mat4 projectionMatrix;
    ...
    static const BT::CPUMemory _nbLights = 6;
    struct Lighting
    {
    ...
    static const BT::uint32   PATCH_SIZE;
    ...
    {
        MouCaGameEngine3DTest::configureEventManager();
    ...
    void makeScene()
    {
        ...
        auto& loader    = _environment.getCore()->getLoaderManager();

        const auto texturePath = resources.getResourceFolder(MouCaCore::IResourceManager::Textures);
    ...
    {
        BT_PRE_CONDITION(!_models._imageHeight->isNull());
        ...
        for (BT::uint32 x = 0; x < PATCH_SIZE; x++)
        {
            for (BT::uint32 y = 0; y < PATCH_SIZE; y++)
            {
                BT::uint32 index = (x + y * PATCH_SIZE);
...

        {
            ...
            const BT::CPUMemory dim      = static_cast<BT::CPUMemory>(imageHeight->getExtents(0).x);
            const auto* heightdata = reinterpret_cast<const BT::uint16*>(imageHeight->getRAWData(0, 0));

            const auto scale = static_cast<BT::uint32>(dim / PATCH_SIZE);

            auto getHeight = [&](BT::uint32 x, BT::uint32 y) -> float
            {
                ...
                const BT::CPUMemory idMem = (rpos.x + rpos.y * dim) * scale;
                BT_ASSERT_BETWEEN(idMem, 0, dim * dim);
        ...
        const BT::uint32 w = (PATCH_SIZE - 1);
        RT::BufferCPUSPtr bufIndices = std::make_shared<RT::BufferCPU>();
        const BT::CPUMemory nbIndices = w * w;
        bufIndices->create(iboDescriptor, nbIndices);
        auto* indices = reinterpret_cast<BT::uint32*>(bufIndices->lock());

        for(BT::uint32 x = 0; x < w; x++)
        {
            for(BT::uint32 y = 0; y < w; y++)
            {
...
const BT::uint32   TessellationTest::PATCH_SIZE = 64;
...
//-------------------------------------------------------------------------------------------------
    ...
    BT_ASSERT(_switchWireframe != nullptr);
