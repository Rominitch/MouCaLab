...
#include "MouCa3DVulkanEngine/include/Engine3DTransfer.h"
#include "MouCa3DVulkanEngine/include/ImGUIManager.h"
...
struct GlyphInstance
{
    ...
    alignas(4) BT::uint32  glyph_index;
...
class FontSVGTest : public MouCaGameEngine3DTest
{
    public:
        ...
        static BT::Path _fontRoboto;
        static BT::Path _fontNoto;
        static BT::Path _fontNotoJP;
        static BT::Path _fontNotoA;
        static BT::Path _fontCeltic;
        static BT::Path _fontMevNo;
...
//                 inst.rect._max.y = y - bbox._max.y * scale;
        ...
        {
            BT_PRE_CONDITION(!text._font->expired());
        ...
        {
            auto& resources = _environment.getCore()->getResourceManager();
            const auto& fontPath = resources.getResourceFolder(MouCaCore::IResourceManager::Fonts);
        ...
        void demoPage(const BT::uint32 idPage, GUI::FontSVGManager& fontSVGManager)
        {
        ...
        bool updateUIOverlay(const Engine3DManager& manager, bool& refreshCommand)
        {
            bool update = false;
            ...
            {
                static BT::uint32 nbGlyphs = 1;
                static const BT::uint32 nbRun = 30; // For stable result
                ...
                if (ImGui::Button("Compute"))
                {
                    ...
                    for (auto run = 0; run < nbRun; ++run)
                    {
                        ...
                        nbGlyphs = static_cast<BT::uint32>(fontManager._ordered.size());
                    }
        ...
        double framePerformance(const Engine3DManager& manager) const
        {
            ...
            const BT::uint32 nbFrames = 350; // Good number of run
...
BT::Path FontSVGTest::_fontRoboto(u8"Roboto-Medium.ttf");

BT::Path FontSVGTest::_fontNoto(u8"NotoSans-Regular.ttf");
BT::Path FontSVGTest::_fontNotoJP(u8"NotoSansCJKjp-Regular.otf"); 
BT::Path FontSVGTest::_fontNotoA(u8"NotoSansArabicUI-Regular.ttf");

BT::Path FontSVGTest::_fontCeltic(u8"Celtic Knot.TTF");
BT::Path FontSVGTest::_fontMevNo(u8"mevno1.ttf");
...
//-------------------------------------------------------------------------------------------------
    ...
    Engine3DXMLLoader loaderGUI(manager);
...
//-------------------------------------------------------------------------------------------------
    ...
    // Transfers data memory to GPU
    {
        ...
        Engine3DTransfer transfer(*context);
...
//-------------------------------------------------------------------------------------------------
    ...
    if (MouCaEnvironment::isDemonstrator())
    {
        ...
        auto demo = [&](const double )
        {
            ...
            GUI.updateMouse(_environment.get3DEngine().getRTPlatform().getMouse());
            ...
            if (updateUIOverlay(manager, refreshCommand) || needUpdateGUI)
            {
                ...
                BT_ASSERT(containerGUI != nullptr);
