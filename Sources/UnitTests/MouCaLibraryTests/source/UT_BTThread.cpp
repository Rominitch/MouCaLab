...
class DemoThread final : public Core::Thread
{
    ...
    public:
        ...
        _info(val), _modif(modif), _end(false)
        {}
...
TEST(BTThread, defaultUse)
{
    ...
    bool valid = false;
    int retry = 0;
    while(retry < 10 && !valid)
    {
        std::this_thread::sleep_for(2ms);
        valid = !myThread.isTerminated();
        ++retry;
    }
...
//--------------------------------------------------------------------------
TEST(BTThread, crashInsideThread)
{
    CrashThread thread;
