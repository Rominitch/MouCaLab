...
TEST(RTMonitor, getter)
{
    ...
    const Core::String name(u8"Virtual Monitor");
...
TEST(RTMonitor, screenRatio)
{
    ...
    RT::Monitor monitor;
    ASSERT_NO_THROW(monitor.initialize(u8"Virtual Monitor", RT::ViewportInt32(0, 0, 1920, 1200), RT::Array2(200.0f, 200.0f), 60, handle));
    EXPECT_EQ(RT::Monitor::Screen16_10, monitor.getScreenRatio());

    ASSERT_NO_THROW(monitor.initialize(u8"Virtual Monitor", RT::ViewportInt32(0, 0, 1920, 1080), RT::Array2(200.0f, 200.0f), 60, handle));
    EXPECT_EQ(RT::Monitor::Screen16_9, monitor.getScreenRatio());

    ASSERT_NO_THROW(monitor.initialize(u8"Virtual Monitor", RT::ViewportInt32(0, 0, 800, 600), RT::Array2(200.0f, 200.0f), 60, handle));
    EXPECT_EQ(RT::Monitor::Screen4_3, monitor.getScreenRatio());

    ASSERT_NO_THROW(monitor.initialize(u8"Virtual Monitor", RT::ViewportInt32(0, 0, 800, 800), RT::Array2(200.0f, 200.0f), 60, handle));
