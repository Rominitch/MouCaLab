...
#include "LibCore/include/BTFileTracker.h"
...
FileTracker::~FileTracker()
{
    BT_PRE_CONDITION( _thread->isNull() || _thread->isTerminated() );
}

void FileTracker::registerResource( Core::ResourceSPtr resource )
{
    BT_PRE_CONDITION( _thread != nullptr );
...
{
    BT_PRE_CONDITION( _thread != nullptr );
    BT_PRE_CONDITION( _thread->isNull() );
    _thread->ready();
    _thread->start();
    ...
    BT_POST_CONDITION( !_thread->isTerminated() );
}

void FileTracker::stopTracking()
{
    BT_PRE_CONDITION( !_thread->isTerminated() );
    _thread->finish();
    _thread->join();
...
{
    BT_PRE_CONDITION( resource != nullptr && !resource->getTrackedFilename().empty() ); // DEV Issue: Need valid resource !
    BT_PRE_CONDITION( _tracked.size() < MAXIMUM_WAIT_OBJECTS );                         // DEV Issue: 64 item tracked for one wait !
    ...
    BT_POST_CONDITION( std::find_if( _tracked.begin(), _tracked.end(), [&]( const std::pair<key, std::vector<FileInfo>>& data ) { return data.first._trackedPath == folder; } ) != _tracked.end() ); // DEV Issue: folder is tracked now !
}
