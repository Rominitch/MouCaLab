...
_handleRead(INVALID_HANDLE_VALUE), _handleWrite(INVALID_HANDLE_VALUE)
{
    ...
    BT_POST_CONDITION(_handleRead  != INVALID_HANDLE_VALUE);
    BT_POST_CONDITION(_handleWrite != INVALID_HANDLE_VALUE);
}
...
{
    BT_PRE_CONDITION(std::filesystem::exists(executable)); // Dev Issue: prefer call program from full absolute path.
}

void Process::execute(const std::vector<Core::StringOS>& arguments)
{
    BT_PRE_CONDITION(_state == State::NotExecuted); //DEV Issue: no re-entrance (maybe more strong ?)
...
{
    BT_PRE_CONDITION(pipe.getHandleRead() != INVALID_HANDLE_VALUE);
