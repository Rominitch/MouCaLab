...
namespace Core
{
    ...
    namespace Maths
    {
        ...
        {
            BT_PRE_CONDITION(!std::numeric_limits<DataType>::is_integer);
            ...
            while(!normalized);
            BT_POST_CONDITION(static_cast<DataType>(0.0) <= angleRad && angleRad < f2PI); //DEV Issue: more than 1 multiple ?
            return angleRad;
        }
