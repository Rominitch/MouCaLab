...
layout(set = 1, binding = 0) uniform UBOTesselation
{
...
float screenSpaceTessFactor(vec4 p0, vec4 p1)
{
    ...
    // View space
    vec4 v0 = uboCamera.view * uboCamera.model  * midPoint;
    
    // Project into clip space
    vec4 clip0 = (uboCamera.projection * (v0 - vec4(radius, vec3(0.0))));
    vec4 clip1 = (uboCamera.projection * (v0 + vec4(radius, vec3(0.0))));
    ...
    clip0.xy *= uboCamera.viewportDim;
    clip1.xy *= uboCamera.viewportDim;
...
bool frustumCheck()
{
    ...
    for (int i = 0; i < 6; i++) {
        if (dot(pos, uboCamera.frustumPlanes[i]) + radius < 0.0)
        {
            return false;
        }
    }
